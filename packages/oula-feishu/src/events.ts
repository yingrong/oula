...
export type FeishuMomEvent = ImmediateEvent | OneShotEvent | PeriodicEvent;
...
export class EventsWatcher {
	...
	/**
	 * Start watching for events. Call this after FeishuBot is ready.
	 */
	start(): void {
	...
	private async handleFile(filename: string): Promise<void> {
		...
		// Parse with retries
		let event: FeishuMomEvent | null = null;
	...
	private parseEvent(content: string, filename: string): FeishuMomEvent | null {
	...
	private execute(filename: string, event: FeishuMomEvent, deleteAfter: boolean = true): void {
		// Format the message
