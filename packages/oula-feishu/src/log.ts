...
function formatContext(ctx: LogContext): string {
	// For Feishu: use chat_id or name
	const id = ctx.chatId || ctx.channelId || "unknown";
	const name = ctx.chatName || ctx.channelName;
	if (name) {
...
export function logStartup(workingDir: string, sandbox: string): void {
	console.log("Starting oula-feishu bot...");
	console.log(`  Working directory: ${workingDir}`);
	console.log(`  Sandbox: ${sandbox}`);
}

export function logConnected(): void {
	console.log("⚡️ Oula-Feishu bot connected and listening!");
	console.log("");
}

export function logDisconnected(): void {
	console.log("Oula-Feishu bot disconnected.");
}
