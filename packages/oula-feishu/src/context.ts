// @ts-nocheck
/**
 * Context management for oula-feishu.
 *
 * Oula-Feishu uses two files per channel:
 ...
 * @param excludeFeishuTs - Feishu timestamp of current message (will be added via prompt(), not sync)
 * @returns Number of messages synced
 */
export function syncLogToSessionManager(
	sessionManager: SessionManager,
	channelDir: string,
	excludeFeishuTs?: string,
): number {
	...
	for (const line of logLines) {
		try {
			...
			const feishuTs = logMsg.ts;
			const date = logMsg.date;
			if (!feishuTs || !date) continue;

			// Skip the current message being processed (will be added via prompt())
			if (excludeFeishuTs && feishuTs === excludeFeishuTs) continue;
...
/**
 * Settings manager for oula-feishu.
