...
describe('NVIDIA Kimi 2.5 集成测试', () => {
  ...
  it('应该能够处理简单问候', async () => {
    ...
    try {
      ...
      console.log(`使用情况: ${response.usage?.promptTokens} 输入 tokens, ${response.usage?.completionTokens} 输出 tokens`);
      console.log('测试结果: 成功');
  ...
  it('应该能够处理多轮对话', async () => {
    ...
    const messages = [
      ...
      { role: 'assistant', content: '抱歉，我无法实时访问天气数据。不过你可以通过天气预报网站或应用程序查询最新的天气信息。' },
